...
> 마지막 점검: **2026-08-12** — **로컬 완결형 MVP 완료** (온라인·외부 애널리틱스만 보류)
...
**현재 포지션:** 브라우저 로컬만으로 플레이·성장·재도전이 도는 **완결형 MVP**.  
...
- [x] 성장: 레벨업 3선택지, 슬롯 5, Tier1→2→3 (9무기/6레시피)  
...
| 2 | `laser` / `railgun` / `swarm` | 레이저 / 레일건 / 스웜 | 조합 상위 |
| 3 | `omega` / `starfall` / `genesis` | 오메가 / 스타폴 / 제네시스 | 종결 |

- 슬롯 5 → 경로 선택형 빌드  
- 조합 레벨 = 재료 중 **낮은 쪽** 계승  
- 가중치: 조합 100 / 강화 40 / 신규 25 / 잭팟 4% (`LevelUpSystem.ts`)
...
| 후보 | 플레이어 체감 | 비용 |
|---|---|---|
| 웨이브/보스/무기 수치 튜닝 | 클리어율·빌드 편중 조절 | 낮음 (`GameConfig`) |
| 일반 적 2~3종 / 웨이브 리듬 | 회피 패턴 다양화 | 낮~중 |
| Tier1 4번째 계열 + 조합 확장 | 빌드 경우의 수↑ | 중 |
| 보스 **페이즈** (HP 구간별 탄막) | 보스전 긴장감 | 중 |
| 드롭 1~2종 (쉴드·버프 등) | 상황 판단 | 낮~중 |
| 세션 길이 A/B (3/5/7분) | 데모·리텐션 실험 | 낮음 |
